...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='请再次输入新密码', 
                               widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder':'请再次输入新的r密码'}))
    ...
    def clean(self):
        ...
        if new_password != new_password_again or new_password == '':
            raise forms.ValidationError('再次密码输入不一致')
        return self.cleaned_data
...
class ForgotPasswordForm(forms.Form):
    email = forms.EmailField(
        label='邮箱',
        widget=forms.EmailInput(
            attrs={'class':'form-control', 'placeholder':'请输入绑定过的的邮箱'}
        )
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError('邮箱s不存在')
        return email
